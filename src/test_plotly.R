...
ui <- fluidPage(
  ...
  column(12, plotlyOutput("thirdplot")),
  column(12, plotlyOutput("subplot")),
  bsModal('boxPopUp1', '', '', plotlyOutput("hover1")),
  bsModal('boxPopUp2', '', '', plotlyOutput("hover2"))
)
...
server <- function(input, output,session) {
  ...
  p2 <- plot_ly(
    ...
    color = ~ symbol, colors = "#cbb255",source = "sub", showlegend = FALSE
  ) %>% layout(
    title = example$symbol[1],
    xaxis = list(
  ...
  observeEvent(event_data("plotly_click", source = "main"), {
    toggleModal(session, modalId = "boxPopUp1", toggle = "toggle")
  })
  observeEvent(event_data("plotly_click", source = "sub"), {
    toggleModal(session, modalId = "boxPopUp2", toggle = "toggle")
  })
  output$hover1 <- renderPlotly({
    eventdat <- event_data('plotly_click', source="main")
  ...
  output$hover2 <- renderPlotly({
    eventdat <- event_data('plotly_click', source="sub")
    if(is.null(eventdat) == T) return(NULL)        # If NULL dont do anything
    point <- as.numeric(eventdat[['pointNumber']]) # Index of the data point being charted
    print(eventdat)
    # draw plot according to the point number on hover
    dat2 <- data.frame(cond = factor(rep("B", each=200)), rating = rnorm(200))
    ggplot(dat2, aes(x=cond, y=rating)) + geom_point()
  })
