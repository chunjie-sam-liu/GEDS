...
other %>%
  ...
  dplyr::mutate(ypos=ifelse(Source == "mRNA_CCLE",0.85,0.5)) %>%
...
data.frame(Project=project,
           ...
           percent = 0.8,
           ypos = 0.75-nchar(project)/90) %>%
  dplyr::as_tibble() -> name.plot

rbind(other_plot_ready,name.plot) -> other_plot_ready

other_plot_ready %>% dplyr::mutate(Source = factor(other_plot_ready$Source,levels = c("protein_black", "protein_MCLP", "mRNA_black", "mRNA_CCLE", "Name"))) -> other_plot_ready
...
ggplot(mapping= aes(x=Project,y=percent,fill=Source)) +
  geom_col(position = "stack")  +
  scale_fill_manual(values = c("#f0f8ff", "#75a3e7", "#f0f8ff", "#f59563","white")) +
  geom_text(aes(label = Cases), y = other_plot_ready$ypos, size = 4 , angle = ang) +
  theme(
    plot.background = element_blank(),
    text = element_text(colour = 'black', size = 20),
